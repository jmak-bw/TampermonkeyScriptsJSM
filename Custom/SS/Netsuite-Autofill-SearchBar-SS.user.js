...
// @updateURL    https://raw.githubusercontent.com/jmak-bw/TampermonkeyScriptsJSM/main/Custom/SS/Netsuite-Autofill-SearchBar-SS.user.js
// @downloadURL  https://raw.githubusercontent.com/jmak-bw/TampermonkeyScriptsJSM/main/Custom/SS/Netsuite-Autofill-SearchBar-SS.user.js
// ==/UserScript==
...
(function() {
    ...
    function adjustYear(input, delta) {
        const pattern = /(SO|INV|PO|JE)-BWUK-(\d{4})(-.*)?/;
    ...
    const buttonObserver = new MutationObserver(() => {
        ...
        if (stackPanel && searchBarContainer) {
            ...
            if (searchInput) {
                ...
                jeButton = document.createElement('button');
                jeButton.textContent = 'JE';

                // Basic styling
                [soButton, invButton, poButton, jeButton].forEach(btn => {
                    Object.assign(btn.style, {
                        padding: '7px',
                        minWidth: '35px',
                        textAlign: 'center',
                ...
                buttonWrapper.appendChild(jeButton);
                ...
                function applyPrefix(prefix, input) {
                    ...
                    const fullPattern = /^(SO|INV|PO|JE)-BWUK-(\d{4})-(.*)$/;
                    const value = input.value.trim();

                    if (fullPattern.test(value)) {
                        input.value = value.replace(/^(SO|INV|PO|JE)-/, `${prefix}-`);
                    } else if (value) {
