// ==UserScript==
// @name         [Netsuite]Autofill Search Bar (CW)
...
// @updateURL    https://raw.githubusercontent.com/jmak-bw/TampermonkeyScriptsJSM/main/Custom/CW/Netsuite-Autofill-SearchBar-CW.user.js
// @downloadURL  https://raw.githubusercontent.com/jmak-bw/TampermonkeyScriptsJSM/main/Custom/CW/Netsuite-Autofill-SearchBar-CW.user.js
// ==/UserScript==
...
(function() {
    ...
    function adjustYear(input, delta) {
        const pattern = /(SO|INV|PO|CD|PMYT|CM)-BWUK-(\d{4})(-.*)?/;
    ...
    const buttonObserver = new MutationObserver(() => {
        ...
        if (stackPanel && searchBarContainer) {
            ...
            if (searchInput) {
                ...
                cdButton = document.createElement('button');
                cdButton.textContent = 'CD';
                pmytButton = document.createElement('button');
                pmytButton.textContent = 'PMYT';
                cmButton = document.createElement('button');
                cmButton.textContent = 'CM';

                // Basic styling
                [soButton, invButton, poButton, cdButton, pmytButton, cmButton].forEach(btn => {
                    Object.assign(btn.style, {
                        padding: '7px',
                        minWidth: '35px',
                        textAlign: 'center',
                ...
                buttonWrapper.appendChild(cdButton);
                buttonWrapper.appendChild(pmytButton);
                buttonWrapper.appendChild(cmButton);
                ...
                function applyPrefix(prefix, input) {
                    ...
                    const fullPattern = /^(SO|INV|PO|CD|PMYT|CM)-BWUK-(\d{4})-(.*)$/;
                    const value = input.value.trim();

                    if (fullPattern.test(value)) {
                        input.value = value.replace(/^(SO|INV|PO|CD|PMYT|CM)-/, `${prefix}-`);
                    } else if (value) {
                ...
                setupMainButton(cdButton, 'CD');
                setupMainButton(pmytButton, 'PMYT');
                setupMainButton(cmButton, 'CM');
            } else {
